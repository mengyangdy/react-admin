...
export function getRecommendedColorPalette(color: string) {
	...
	if (!mainColor) {
		throw new Error("Main color (500) not found in color palette");
	}
	const matchColor = colorPaletteFamily.palettes.find((palette) => palette.hex === color);
	if (!matchColor) {
		throw new Error(`Match color not found for color: ${color}`);
	}
...
export function getRecommendedPaletteColorByNumber(color: string, number: ColorPaletteNumber) {
	...
	const palette = colorPalette.colorMap.get(number);
	if (!palette) {
		throw new Error(`Color palette number ${number} not found`);
	}
