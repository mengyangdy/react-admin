...
const AdminLayout: FC<AdminLayoutProps> = ({
	...
	const isWrappperScroll = scrollMode === "wrapper";
	const isContentScroll = scrollMode === "content";

	const isVertical = mode === "vertical";
	const isHorizontal = mode === "horizontal";
	const fixedHeaderAndTab = fixedTop || (isHorizontal && isWrappperScroll);
	...
	const footerLeftGapClass = () => {
		...
		const condition2 = isHorizontal && isWrappperScroll && !fixedFooter;
	...
	return (
		<section
			className={classNames("reactive h-full", commonClass)}
			style={cssVar}
		>
			<section
				id={isWrappperScroll ? scrollElId : ""}
				className={classNames("h-full flex flex-col", scrollWrapperClass, commonClass, {
					"overflow-y-auto": isWrappperScroll,
				})}
			>
				{showHeader && (
					<>
						<header
							style={{
								display: fullContent ? "none" : "block",
							}}
				...
				{showTab && (
					<>
						...
						<div
							className={classNames("flex-shrink-0 overflow-hidden", [
				...
				<main
					...
					className={classNames(
						"flex flex-col flex-grow bg-layout",
						commonClass,
						contentClass,
						leftGapClass(),
						{
