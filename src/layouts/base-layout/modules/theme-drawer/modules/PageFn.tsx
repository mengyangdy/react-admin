...
const PageFn = memo(() => {
	...
	return (
		<div className="relative flex-col-stretch gap-12px">
			...
			<SettingItem
				label="标签栏高度"
				show={themeSettings.tab.visible}
			>
				<InputNumber
					className="w-120px"
			...
			>
				<InputNumber
					...
					onChange={(value) => themeActions.setSider({ collapsedWidth: value ?? 0 })}
				/>
			</SettingItem>
			...
			>
				<InputNumber
					...
					onChange={(value) => themeActions.setSider({ mixCollapsedWidth: value ?? 0 })}
				/>
			</SettingItem>
			...
			>
				<InputNumber
					...
					onChange={(value) => themeActions.setSider({ mixChildMenuWidth: value ?? 0 })}
				/>
			</SettingItem>
			...
			<SettingItem
				label="底部居右"
				show={Boolean(themeSettings.footer.visible && layoutMode === "horizontal-mix")}
			>
				<Switch
			...
			>
				<Input
					...
					onChange={(e) => themeActions.setWatermark({ text: e.target.value || "" })}
				/>
			</SettingItem>
		</div>
	);
