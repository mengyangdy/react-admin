...
const GlobalTab = () => {
  ...
  const { activeTabId, isTabRetain, navigate, removeTabById, tabs } =
    useTabActions();
  ...
  function getContextMenuDisabledKeys(
    tabId: string,
    index: number
  ): App.Global.DropdownKey[] {
  ...
  return (
    <DarkModeContainer className="size-full flex-y-center px-16px shadow-tab">
          ...
          >
            {tabs.map((item, index) => (
              ...
              >
                <div
                  className={
                    themeSettings.tab.mode === "slider" ? "h-full" : undefined
                  }
                  id={item.id}
                >
                  <PageTab
      ...
      <TabReloadButton />
      <FullScreen full={fullContent} toggleFullscreen={toggleContent} />
    </DarkModeContainer>
  );
};
