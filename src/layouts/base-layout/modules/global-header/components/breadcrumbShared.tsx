...
function BreadcrumbContent({
  ...
  return (
    <div className="i-flex-y-center align-middle">
      {cloneElement(icon, { className: "mr-4px text-icon" } as Record<
        string,
        unknown
      >)}
      {label}
...
export function getBreadcrumbsByRoute(
  routeData: ReturnType<
    typeof import("@/hooks/router/useCurrentRoute").useCurrentRoute
  >,
  ...
  for (let i = 1; i < matched.length; i++) {
    ...
    const currentMenu = currentMenus.find(
      (item) => item?.key === matchedRoute.pathname
    );
