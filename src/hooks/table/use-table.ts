...
type Config<A extends AntDesign.TableApiFn> = Omit<AntDesign.AntDesignTableConfig<A>, "columns"> & {
	queryKey: QueryKey; // 必传，用于 React Query 缓存
	columns: (data: unknown) => TableColumn<AntDesign.TableDataWithIndex<GetTableData<A>>>[];
};
	...
	const {
		...
		getColumns: (cols, checks) => {
			...
			const filteredColumns = checks
				...
				.filter((col): col is TableColumn<AntDesign.TableDataWithIndex<GetTableData<A>>> => !!col);
		...
		transformer: (res) => {
			const response = res as
				...
				| null
				| undefined;
			const { current = 1, records = [], size = 10, total: totalNum = 0 } = response || {};

			const recordsWithIndex: AntDesign.TableDataWithIndex<GetTableData<A>>[] = records.map(
				(item, index) => {
					return {
...
export function useTableOperate<T extends TableData = TableData>(
	...
	executeResActions: (res: T, operateType: AntDesign.TableOperateType) => Promise<void> | void,
) {
	...
	function handleEdit(idOrData: T["id"] | T) {
		...
		if (currentItem) {
			form.setFieldsValue(currentItem as Parameters<typeof form.setFieldsValue>[0] & T);
