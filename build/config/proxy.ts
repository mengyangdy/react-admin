...
function createProxyItem(item: App.Service.ServiceConfigItem, enableLog: boolean) {
  ...
  proxy[item.proxyPattern] = {
    ...
    configure: (_proxy: HttpProxy.ProxyServer, options: ProxyOptions) => {
      _proxy.on('proxyReq', (_proxyReq: ClientRequest, req: IncomingMessage, _res: ServerResponse) => {
        if (!enableLog) return;
      ...
      _proxy.on('error', (_err: Error, req: IncomingMessage, _res: ServerResponse | Socket) => {
        if (!enableLog) return;
